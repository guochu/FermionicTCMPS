...
changepositions(x::FreeFermionicHamiltonian, m::AbstractDict{Int, Int}) = FreeFermionicHamiltonian([changepositions(item, m) for item in x.data])
...
function coefficient_matrix(x::FreeFermionicHamiltonian, L::Int)
	@assert (length(x) <= L)
	@assert isconstant(x)
	m = zeros(scalartype(x), L, L)
	for item in x.data
		i, j = positions(item)
		m[i, j] = scalar(coeff(item))
	end
	return m
end
